...
recession_rate <- function(conn, ow_uid, dtime, level_ft) {
  ...
  # filtering rain data
  boundaries <- group_by(level_ft_df, ow_uid) %>%
    summarize(
      start = min(dtime),
      end = max(dtime)
    )
  ...
  gage <- dbGetQuery(conn, paste0(
    "select * from admin.tbl_smp_gage
                                           left join fieldwork.tbl_ow using(smp_id) where ow_uid in (",
  ...
  time_grid <- data.frame(dtime = seq(
    floor_date(min(output_df$dtime, na.rm = TRUE), "15 mins"),
    ceiling_date(max(output_df$dtime, na.rm = TRUE), "15 mins"),
    by = "15 mins"
  ))
